/**
 * Gen 3 StandaloneFormat (.pk3) — Phase 2.1.
 *
 * Wires the existing PK3 crypto (entity.ts) and block shuffle (entityFormat.ts)
 * to the IStandalonePokemonFormat interface.
 */
...
import { getNatureName, getAbilitySlot, getGenderFromPid, extractGen3IVs, packGen3IVs } from './identity';
...
export class Gen3StandaloneFormat implements IStandalonePokemonFormat {
  ...
  createFile(mon: PokemonStats): Uint8Array {
    ...
    // Nickname (0x08, 10 bytes) — Gen 3 uses a custom charmap; for now use raw bytes
    // Phase 2.2: implement Gen 3 text codec
    const nick = mon.nickname || mon.speciesName || '??????????';
    for (let i = 0; i < 10 && i < nick.length; i++) {
      buf[8 + i] = nick.charCodeAt(i) & 0xFF;
    }

    // Language (0x12, 1 byte) — English = 2 (E)
    buf[0x12] = 2;

    // OT Name (0x13, 7 bytes)
    const otName = mon.originalTrainerName || '???????';
    for (let i = 0; i < 7 && i < otName.length; i++) {
      buf[0x13 + i] = otName.charCodeAt(i) & 0xFF;
    }
    ...
    // Attacks block (12 bytes): moves(u16×4)
    for (let i = 0; i < 4; i++) {
  ...
  parseFile(buffer: Uint8Array): PokemonStats {
    ...
    // Parse nickname (raw bytes — Phase 2.2 will add Gen 3 text codec)
    let nickname = '';
    for (let i = 0; i < 10; i++) {
      const b = bytes[8 + i]!;
      if (b === 0xFF) break; // terminator
      nickname += String.fromCharCode(b);
    }

    // Parse OT name
    let otName = '';
    for (let i = 0; i < 7; i++) {
      const b = bytes[0x13 + i]!;
      if (b === 0xFF) break;
      otName += String.fromCharCode(b);
    }
    ...
    const level = 100; // Phase 2.2: derive from exp
    ...
    if (base) {
      const mon: PokemonStats = {
        ...
        moveIds: moves, movePp: [0, 0, 0, 0], movePpUps: [0, 0, 0, 0],
        status: 'OK', catchRate: 0,
        type1: typeInfo.type1, type2: typeInfo.type2,
        type1Name: typeInfo.type1Name, type2Name: typeInfo.type2Name,
        isParty: true, isEgg: false,
        isShiny: false, gender, heldItemId: heldItem, heldItemName: heldItem ? `Item ${heldItem}` : undefined,
