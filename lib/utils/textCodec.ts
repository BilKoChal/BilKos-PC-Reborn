/**
 * Shared Game Boy text encoding/decoding utilities.
 * Used by both Gen 1 and Gen 2 adapters since they share the same character encoding.
 */

/**
 * Reverse character map: human-readable character → Game Boy internal byte value.
 * Covers the Latin alphabet (A-Z, a-z), digits (0-9), basic punctuation, and space.
 */
export const CHAR_MAP_REV: Record<string, number> = {
...
/**
 * Encodes a text string into Game Boy internal byte representation.
 * @param text The string to encode
 * @param length The target buffer length
 * @param terminator The terminator byte (default 0x50)
 * @returns Uint8Array of encoded bytes
 */
export function encodeGameBoyText(text: string, length: number, terminator: number = 0x50): Uint8Array {
